...
@pytest.fixture
def fresh_control_plane(tmp_path: Path) -> IngestionConfigRepository:
    """Provide a fresh control plane store with automatic cleanup."""
    return IngestionConfigRepository(tmp_path / "config.db")


def build_catalog(control_plane: IngestionConfigRepository | None):
    """Build a publication contract catalog with standard packs and configuration."""
    return build_publication_contract_catalog(
...
def test_export_contracts_enriches_with_confidence_fields_when_snapshot_exists(
    fresh_control_plane: IngestionConfigRepository,
) -> None:
    ...
    fresh_control_plane.record_publication_confidence_snapshot((snapshot,))

    # 3. Build publication contract catalog with control plane
    catalog = build_catalog(fresh_control_plane)
...
def test_export_contracts_has_none_confidence_fields_without_snapshot(
    fresh_control_plane: IngestionConfigRepository,
) -> None:
...
def test_export_contracts_with_multiple_publication_snapshots(
    fresh_control_plane: IngestionConfigRepository,
) -> None:
    ...
    fresh_control_plane.record_publication_confidence_snapshot(snapshots)
