...
def test_agent_runbooks_document_preflight_and_degraded_paths() -> None:
    ...
    # `dispatch-review/SKILL.md` is generated by `sync_skills.py` from the
    # canonical agentops template, which carries no degraded-review guidance; the
    # text these fragments asserted was removed by 72dac27 and exists nowhere in
    # the tree, including `.agents/overlays/`. The degraded paths this test is
    # named for are still asserted above, against the runbooks this repo owns.
    # If degraded-review guidance is wanted back, it belongs in the agentops
    # template or in an overlay -- asserting it against a synced file here only
    # re-reds `verify-fast`.
