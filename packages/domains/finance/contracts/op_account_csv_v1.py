"""OP account transaction CSV contract parser."""
...
CONTRACT_ID = "op_account_transactions_csv_v1"
DATASET_TYPE = FinanceDatasetType.TRANSACTION_EVENT_STREAM

_SOURCE_HEADER_ALIASES: dict[str, tuple[str, ...]] = {
    "booked_at": ("Kirjauspäivä", "booked_at"),
    "value_date": ("Arvopäivä", "value_date"),
    "account_id": ("Tilinumero", "account_id"),
    "counterparty_name": ("Saaja/Maksaja", "counterparty_name"),
    "amount": ("Summa", "amount"),
    "currency": ("Valuutta", "currency"),
    "description": ("Viesti", "description"),
    "archive_id": ("Arkistotunnus", "archive_id"),
    "provider_state": ("Tapahtuman tila", "provider_state"),
    "provider_type": ("Tapahtumalaji", "provider_type"),
}

_REPAYMENT_PATTERNS = (
    re.compile(
        r"(?P<label>pääoma|principal)\s+(?P<principal>-?\d+[.,]\d{2}).*?"
        r"(?P<label2>korko|interest)\s+(?P<interest>-?\d+[.,]\d{2}).*?"
        r"(?P<label3>kulut|fees?)\s+(?P<fees>-?\d+[.,]\d{2}).*?"
        r"(?P<label4>jäljellä|remaining)\s+(?P<remaining>-?\d+[.,]\d{2})",
        re.IGNORECASE,
    ),
)
...
class OPAccountTransactionRecord:
    booked_at: date
    booking_month: str
    ...
    repayment_principal: Decimal | None = None
    repayment_interest: Decimal | None = None
    repayment_fees: Decimal | None = None
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "repayment_principal": self.repayment_principal,
            "repayment_interest": self.repayment_interest,
            "repayment_fees": self.repayment_fees,
            "remaining_balance": self.remaining_balance,
        }


class OPAccountTransactionCsvParser:
    """Parse OP bank account CSV exports into canonical transaction rows."""
    ...
    def detect(self, file_name: str, header_bytes: bytes) -> bool:
        ...
        return "Kirjauspäivä" in normalized_header or "booked_at" in normalized_header
    ...
    def parse(self, source_bytes: bytes, file_name: str) -> ParseResult:
        ...
        if not source_header:
            warnings.append(
                ValidationIssue(
                    code="empty_file",
                    message="OP account CSV content is empty.",
                )
            )
            return ParseResult(
        ...
        missing_columns = [
            canonical_name
            for canonical_name, aliases in _SOURCE_HEADER_ALIASES.items()
            if canonical_name in {"booked_at", "account_id", "counterparty_name", "amount", "currency"}
            and not _has_any_column(source_header, aliases)
        ]
        for canonical_name in missing_columns:
            warnings.append(
                ValidationIssue(
                    code="missing_required_column",
                    message=f"Required OP CSV column '{canonical_name}' is missing.",
        ...
        metadata = {
            ...
            "source_format": "op-semicolon-csv",
        }
        return ParseResult(
            dataset_type=self.dataset_type,
            records=records,
        ...
        if result.dataset_type != self.dataset_type:
            issues.append(
                ValidationIssue(
                    ...
                    message=(
                        f"Expected dataset type '{self.dataset_type.value}' for OP account CSV."
                    ),
                )
            )
        ...
        if not isinstance(source_header, list) or not source_header:
            issues.append(
                ValidationIssue(
                    ...
                    message="OP account CSV parse result is missing source header metadata.",
                )
            )
...
def load_op_account_transactions_bytes(source_bytes: bytes) -> list[dict[str, Any]]:
    parser = OPAccountTransactionCsvParser()
    return parser.parse(source_bytes, "op-account-export.csv").records


def _parse_row(row: dict[str, str | None]) -> OPAccountTransactionRecord:
    ...
    return OPAccountTransactionRecord(
        booked_at=booked_at,
        ...
        repayment_principal=repayment.get("repayment_principal"),
        repayment_interest=repayment.get("repayment_interest"),
        repayment_fees=repayment.get("repayment_fees"),
        remaining_balance=repayment.get("remaining_balance"),
    )


def _extract_repayment_details(description: str) -> dict[str, Decimal | None]:
    for pattern in _REPAYMENT_PATTERNS:
        match = pattern.search(description)
        if match is None:
            continue
        return {
            "repayment_principal": _parse_decimal_text(match.group("principal")),
            "repayment_interest": _parse_decimal_text(match.group("interest")),
            "repayment_fees": _parse_decimal_text(match.group("fees")),
            "remaining_balance": _parse_decimal_text(match.group("remaining")),
        }
    return {
        "repayment_principal": None,
...
def _get_value(row: dict[str, str | None], canonical_name: str) -> str:
    ...
    if value is None:
        raise ValueError(f"Missing required OP CSV field '{canonical_name}'.")
    return value
...
def _parse_date(row: dict[str, str | None], canonical_name: str) -> date:
    ...
    try:
        ...
        raise ValueError(f"Invalid OP CSV date for '{canonical_name}': {raw_value!r}") from exc
...
def _parse_decimal_text(raw_value: str) -> Decimal:
    ...
    try:
        ...
        raise ValueError(f"Invalid OP CSV decimal value: {raw_value!r}") from exc
