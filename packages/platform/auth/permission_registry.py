...
PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ = "control.schedule_dispatches.read"
PERMISSION_CONTROL_SCHEDULE_DISPATCHES_WRITE = "control.schedule_dispatches.write"
PERMISSION_CONTROL_ACTION_PROPOSALS_WRITE = "control.action_proposals.write"
PERMISSION_CONTROL_CONFIG_READ = "control.config.read"
PERMISSION_CONTROL_CONFIG_WRITE = "control.config.write"
PERMISSION_TRANSFORMATION_AUDIT_READ = "transformation.audit.read"
PERMISSION_ADMIN_WRITE = "admin.write"
PERMISSION_CONTROL_POLICY_READ = "control.policy.read"
PERMISSION_CONTROL_POLICY_WRITE = "control.policy.write"
...
PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_SCHEDULE_PREFIX = (
    f"{PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ}.schedule."
)
PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_DISPATCH_PREFIX = (
    f"{PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ}.dispatch."
)
PERMISSION_CONTROL_SCHEDULE_DISPATCHES_WRITE_DISPATCH_PREFIX = (
    f"{PERMISSION_CONTROL_SCHEDULE_DISPATCHES_WRITE}.dispatch."
)
PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_SCHEDULE_WILDCARD = (
    f"{PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_SCHEDULE_PREFIX}*"
)
PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_DISPATCH_WILDCARD = (
    f"{PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_DISPATCH_PREFIX}*"
)
PERMISSION_CONTROL_SCHEDULE_DISPATCHES_WRITE_DISPATCH_WILDCARD = (
    f"{PERMISSION_CONTROL_SCHEDULE_DISPATCHES_WRITE_DISPATCH_PREFIX}*"
)
PERMISSION_CONTROL_CONFIG_READ_RESOURCE_PREFIX = (
    f"{PERMISSION_CONTROL_CONFIG_READ}.resource."
)
PERMISSION_CONTROL_CONFIG_WRITE_RESOURCE_PREFIX = (
    f"{PERMISSION_CONTROL_CONFIG_WRITE}.resource."
)
PERMISSION_CONTROL_CONFIG_READ_RESOURCE_WILDCARD = (
    f"{PERMISSION_CONTROL_CONFIG_READ_RESOURCE_PREFIX}*"
)
PERMISSION_CONTROL_CONFIG_WRITE_RESOURCE_WILDCARD = (
    f"{PERMISSION_CONTROL_CONFIG_WRITE_RESOURCE_PREFIX}*"
)
...
_SERVICE_SCOPE_PERMISSIONS: dict[str, frozenset[str]] = {
    ...
    SERVICE_TOKEN_SCOPE_HA_BRIDGE_INGEST: frozenset({PERMISSION_INGEST_WRITE}),
    SERVICE_TOKEN_SCOPE_ADMIN_WRITE: frozenset(
        {
            PERMISSION_ADMIN_WRITE,
            PERMISSION_CONTROL_CONFIG_READ,
            PERMISSION_CONTROL_CONFIG_WRITE,
...
def schedule_dispatch_read_schedule_permission(schedule_id: str) -> str | None:
    normalized = schedule_id.strip().lower()
    if not normalized:
        return None
    if not _DYNAMIC_PERMISSION_PATTERN.fullmatch(normalized):
        return None
    return f"{PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_SCHEDULE_PREFIX}{normalized}"


def schedule_dispatch_read_dispatch_permission(dispatch_id: str) -> str | None:
    normalized = dispatch_id.strip().lower()
    if not normalized:
        return None
    if not _DYNAMIC_PERMISSION_PATTERN.fullmatch(normalized):
        return None
    return f"{PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_DISPATCH_PREFIX}{normalized}"


def schedule_dispatch_write_dispatch_permission(dispatch_id: str) -> str | None:
    normalized = dispatch_id.strip().lower()
    if not normalized:
        return None
    if not _DYNAMIC_PERMISSION_PATTERN.fullmatch(normalized):
        return None
    return f"{PERMISSION_CONTROL_SCHEDULE_DISPATCHES_WRITE_DISPATCH_PREFIX}{normalized}"


def config_read_resource_permission(resource_key: str) -> str | None:
    normalized = resource_key.strip().lower()
    if not normalized:
        return None
    if not _DYNAMIC_PERMISSION_PATTERN.fullmatch(normalized):
        return None
    return f"{PERMISSION_CONTROL_CONFIG_READ_RESOURCE_PREFIX}{normalized}"


def config_write_resource_permission(resource_key: str) -> str | None:
    normalized = resource_key.strip().lower()
    if not normalized:
        return None
    if not _DYNAMIC_PERMISSION_PATTERN.fullmatch(normalized):
        return None
    return f"{PERMISSION_CONTROL_CONFIG_WRITE_RESOURCE_PREFIX}{normalized}"
...
def _granted_permission_satisfies_required(granted: str, required: str) -> bool:
    ...
    if (
        ...
        return True
    if granted == PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ and (
        required.startswith(PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_SCHEDULE_PREFIX)
        or required.startswith(PERMISSION_CONTROL_SCHEDULE_DISPATCHES_READ_DISPATCH_PREFIX)
    ):
        return True
    if (
        granted == PERMISSION_CONTROL_SCHEDULE_DISPATCHES_WRITE
        and required.startswith(PERMISSION_CONTROL_SCHEDULE_DISPATCHES_WRITE_DISPATCH_PREFIX)
    ):
        return True
    if (
        granted == PERMISSION_CONTROL_CONFIG_READ
        and required.startswith(PERMISSION_CONTROL_CONFIG_READ_RESOURCE_PREFIX)
    ):
        return True
    if (
        granted == PERMISSION_CONTROL_CONFIG_WRITE
        and required.startswith(PERMISSION_CONTROL_CONFIG_WRITE_RESOURCE_PREFIX)
    ):
        return True
    if (
