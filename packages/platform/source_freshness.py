"""Generic source-freshness state computation for source assets."""
...
def _has_coverage_gap(observations: Sequence[SourceFreshnessRunObservation]) -> bool:
    ...
    for current in successful[1:]:
        ...
        if current.covered_from > previous.covered_through + timedelta(days=1):
            return True
        if (
            current.covered_through is not None
            and current.covered_through > previous.covered_through
        ):
            previous = current
    return False
...
def _next_expected_at(
    ...
    if config.expected_frequency == "monthly":
        ...
        next_due = date(
            due_date.year + 1,
            due_date.month,
            min(due_date.day, monthrange(due_date.year + 1, due_date.month)[1]),
        )
    else:
...
def _last_day_of_previous_month(as_of_date: date) -> date:
    ...
    return date(
        as_of_date.year,
        previous_month,
        monthrange(as_of_date.year, previous_month)[1],
    )
