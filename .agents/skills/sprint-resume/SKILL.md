...
4. Check whether an active exclusive claim already exists:
   - If no claim exists, create one with `sprintctl claim create` before repo edits when ownership would otherwise be ambiguous and record a strong identity: `claim_id`, `claim_token`, `runtime_session_id`, `instance_id`, actor, and workspace metadata.
