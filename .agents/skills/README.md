...
- `sprint-packet`: use when accepted scope needs to become a sprint-ready implementation packet with deliverables, acceptance, and verification. Includes registering the sprint and items in sprintctl, with backlog/archive kinds when appropriate.
...
- `sprint-snapshot`: use to render and commit the current sprintctl sprint state as a reviewable plaintext snapshot, with the project DB loaded explicitly first.
