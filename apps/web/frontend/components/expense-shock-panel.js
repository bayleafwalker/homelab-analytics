...
export function ExpenseShockPanel() {
  ...
  async function handleSubmit(e) {
    ...
    const body = { expense_pct_delta: pctDecimal };
    if (label) body.label = label;

    try {
      const res = await fetch("/api/scenarios/expense-shock", {
        method: "POST",
      ...
      {open && (
        <>
          <form className="formGrid fourCol" onSubmit={handleSubmit}>
            <div className="field">
              <label htmlFor="expense-pct">Expense change (%)</label>
              <input
                id="expense-pct"
                type="number"
                step="0.1"
                ...
                required
              />
              <span className="fieldHint">Positive = cost increase, negative = deflation</span>
            </div>
            <div className="field">
              ...
              <input
                id="expense-label"
                type="text"
                placeholder="e.g. Tariff shock 2026"
                value={label}
            ...
            <div className="stack" style={{ marginTop: "1rem" }}>
              <div className="metaGrid">
                <div className="metaItem">
                  <div className="metricLabel">Expense change</div>
                  <div className={`metricValue ${isIncrease ? "status-failed" : ""}`}>
                    {pctNum >= 0 ? "+" : ""}{pctNum.toFixed(1)}%
                  </div>
                </div>
                <div className="metaItem">
                  <div className="metricLabel">Baseline monthly expense</div>
                  <div className="metricValue">{fmt(result.baseline_monthly_expense)}</div>
                </div>
                <div className="metaItem">
                  <div className="metricLabel">New monthly expense</div>
                  <div className={`metricValue ${isIncrease ? "status-failed" : ""}`}>
                    {fmt(result.new_monthly_expense)}
                  </div>
                </div>
                <div className="metaItem">
                  ...
                  <div className={`metricValue ${Number(result.annual_additional_cost) > 0 ? "status-failed" : ""}`}>
                    {Number(result.annual_additional_cost) > 0 ? "+" : ""}{fmt(result.annual_additional_cost)}
                  </div>
                </div>
                ...
                {result.months_until_deficit == null && isIncrease && (
                  <div className="metaItem">
                    ...
                    <div className="metricValue" style={{ color: "var(--ok)" }}>None (income covers new expenses)</div>
                  </div>
                )}
              </div>
