...
- runtime auth/identity mode is passed through as `HOMELAB_ANALYTICS_AUTH_MODE` plus `HOMELAB_ANALYTICS_IDENTITY_MODE`, so the login page can render explicit break-glass local posture versus OIDC sign-in without adding product logic to the Python launcher
