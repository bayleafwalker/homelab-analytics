...
**Phase:** 2
**Status:** in-progress (the Next.js shell now covers local login, OIDC sign-in/callback, dashboard, reporting, auth/security admin for local users and service tokens, control-plane catalog edit/deactivate flows, execution-control queue actions, and filterable run views; it consumes the API only and replaces the old server-rendered Python dashboard, but broader product surface work is still pending)
