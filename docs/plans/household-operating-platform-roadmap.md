...
- Remaining Stage 1 carryover narrowed to the real unfinished dimension and publication semantics: `dim_household_member` plus the explicit reporting/publication follow-up now tracked in `docs/architecture/semantic-contracts.md`; `fact_balance_snapshot` is already implemented as the Stage 1 point-in-time balance fact
...
- Publication-semantic guidance for the current-dimension path and any remaining reporting/publication follow-up
...
Mostly complete. Sprint I finance ingestion is complete, and the worktree now includes internal-platform ingestion, utilities automation foundations, loan and budget planning models, infrastructure metrics dimensions/facts, home-automation state foundations, and the completed asset inventory foundation. Remaining Stage 1 work is the true carryover: semantic-governance cleanup, publication-semantic follow-up, and `dim_household_member`.
...
Complete (2026-09-01, sprints ember-rule-keel and slate-dial-hearth). The HA integration infrastructure is functional: batch entity ingest, WebSocket live subscription, MQTT synthetic entity publication, outbound action dispatcher, approval-gated device control, approval queue publication/UI controls, and tariff/cost/maintenance synthetic entities are all in place. The policy registry is persisted, wired into production evaluation with explicit authority semantics, and covered end-to-end. The operator authoring surface shipped with slate-dial-hearth: an operator now authors, enables, disables and deletes a policy entirely in the UI at `/control/policies`, sees its verdict with the reason behind it, and can preview a rule before saving it.
...
### Stage A2 operator-surface acceptance criteria

From the rev-4 brief (`docs/plans/kotona-operating-picture-brief.md`).

1. **An operator authors, enables and disables a policy entirely in the UI** — ✅ Met. `apps/web/frontend/app/control/policies/page.js` with BFF handlers under `create/`, `[policyId]/`, `[policyId]/enabled/` and `[policyId]/delete/` (`tests/test_web_auth.py::test_nextjs_frontend_policy_mutation_routes_follow_the_bff_pattern`).
2. **It fires through the production path and its verdict is visible with its reason** — ✅ Met, and the reason had to be built: `PolicyResult` carried no reason field, and `value` was doing double duty as both the observed measurement and the stale-input explanation. The two are now distinct, and the four `unavailable` causes are separately diagnosable (`tests/test_ha_policy.py::DeclarativeRuleReasonTests`).
3. **A template with missing required inputs cannot be enabled** — ✅ Met at both layers. The form marks required inputs and refuses submission (`apps/web/frontend/playwright/storybook-ui.spec.js`, story `control-plane-policyform--template-requires-threshold`), and an incomplete rule is refused by the API (`tests/test_policy_api_routes.py::ShippedPolicyTemplateTests::test_incomplete_templates_are_refused`).
4. **No policy appears twice** — ✅ Met. Registry rows and code built-ins are partitioned by id on the page, and a registry row can never shadow a builtin id (`tests/test_ha_policy.py::BuiltinParityTests`).
5. **Only honestly expressible templates ship** — ✅ Met, with the shipped set verified against the real contracts rather than asserted (`tests/test_policy_api_routes.py::ShippedPolicyTemplateTests`). All three read `household_overview`, not `monthly_cashflow`; see "Template discipline" in `policy-and-automation.md` for why, and `TEMPLATE_EXCLUSIONS` for what was rejected.
6. **Typed via the committed OpenAPI export flow, codegen checks green** — ✅ Met. `generated/openapi.json` and `generated/api.d.ts` regenerated for the two new routes; `make verify-fast` passes at 2039 tests.

**Supporting backend work that A2 acceptance required:** an explicit `reason` on every result; `GET /control/policies/referenceable-publications`, because `/contracts/publications` advertises 14 current-dimension keys that policy create rejects and a picker driven off it would offer unusable options; `POST /control/policies/preview`; and the retirement of `in`/`not_in`, `verdict_mapping` and `allowed_freshness_states`, which validated and persisted but never did anything — `in` in particular meant a rule could save cleanly and then evaluate `ok` forever.

**Known follow-up (not A2 scope):** the Storybook screenshot baselines are CI-owned and currently fail on this workstation — an environmental render-height difference, plus a real +501px change from the new retro Policies nav entry. Both need regenerating on the runner that owns them.
