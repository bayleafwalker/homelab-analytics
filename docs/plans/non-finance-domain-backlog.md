...
| Homelab value loop | Go, mostly landed | The homelab pack already owns sources, marts, API routes, web surfaces, HA publication, a saved homelab cost/benefit scenario, and a saved-scenario comparison workflow. The remaining gap is richer scenario sets and saved-set management. | Keep it tied to Stage 3/4 decision surfaces, not another foundation rewrite. |
