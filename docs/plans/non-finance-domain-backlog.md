...
| Homelab value loop | Go now, but narrow | The homelab pack already owns sources, marts, API routes, web surfaces, and HA publication. The missing piece is richer homelab cost/benefit scoring and scenario comparison. | Keep it tied to Stage 3/4 decision surfaces, not another foundation rewrite. |
