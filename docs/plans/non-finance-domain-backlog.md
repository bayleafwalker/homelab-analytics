...
| Homelab value loop | Go, mostly landed | The homelab pack already owns sources, marts, API routes, web surfaces, HA publication, and a saved homelab cost/benefit scenario. The remaining gap is richer scenario sets and operator comparison workflows. | Keep it tied to Stage 3/4 decision surfaces, not another foundation rewrite. |
