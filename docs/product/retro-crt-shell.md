...
**Resolved by:** `docs/decisions/web-surface-shell-decision.md` (sprint `choose-web-surface`, items #716/#717).

**Outcome:** the first of the three named outcomes below — retro stays an explicitly experimental/secondary operator renderer with no parity promise. The default shell remains primary. See the ADR for the hands-on comparison this was based on, the rationale for not harvesting/retiring retro, and the canonical navigation/data-fetching model that now governs how new publication-backed pages must avoid diverging between shells.

The three outcomes that were on the table:

- keep `/retro` as an explicitly experimental operator renderer with no parity promise — **selected**
...
New product workflows continue to land in the default shell first. Porting a feature into retro afterward is optional follow-up work, not a parity requirement. Any new publication-backed page, in either shell, must follow the navigation-registry and server-fetch conventions defined in the ADR.
