...
This document provides the product intent for specific stages of the 11-stage household operating platform roadmap defined in `docs/decisions/household-operating-platform-direction.md`. Stage 5, policy, automation, and action engine, is where the integration surface between the platform and HA is implemented: the semantic bridge, bidirectional event fabric, action dispatch, and approval flows. Stage 6, integration adapter layer, is where the HA-specific integration is positioned as the reference implementation of a generic adapter model. HA protocol choices remain HA-specific; the adapter contracts ensure new integration surfaces can be added without reimplementing the full integration stack. Stage 7, multi-renderer delivery, is where HA entities and cards become first-class renderers of platform publications, allowing platform outputs to appear natively in HA dashboards, automations, and voice surfaces. This document defines what those stages should satisfy from a product standpoint.
