...
**Status:** Active — scenario storage and five compute types shipped, plus shared saved compare sets
...
The frontend renders this as an "Assumption summary" panel beside every scenario comparison view. No computed result is shown without its assumptions. The compare surface also allows shared saved compare sets so operators can reopen recurring comparisons without reselecting both sides.
