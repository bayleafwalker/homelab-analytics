...
- Confirm whether an existing exclusive claim clearly belongs to the current workspace before planning implementation under that item.
...
- Stop if a sprint-scoped item is already exclusively claimed and ownership cannot be proven to match the current workspace.
